const fs = require('fs');

const processFile = (filename) => {
    if (!fs.existsSync(filename)) return;
    let html = fs.readFileSync(filename, 'utf8');
    
    // Replace the grid container with a horizontal scrolling container
    html = html.replace(
        '<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 px-6 md:px-12 pb-8">',
        '<div class="flex overflow-x-auto no-scrollbar gap-6 px-6 md:px-12 pb-8 snap-x snap-mandatory">'
    );

    // Make sure each card has a min-width and snap-align so they work well in the horizontal scroll
    // The cards are currently `<a href="shop.html" class="block group cursor-pointer gsap-featured-card opacity-0 translate-y-12">`
    html = html.replaceAll(
        '<a href="shop.html" class="block group cursor-pointer gsap-featured-card opacity-0 translate-y-12">',
        '<a href="shop.html" class="min-w-[280px] md:min-w-[320px] lg:min-w-[400px] snap-center block group cursor-pointer gsap-featured-card opacity-0 translate-y-12">'
    );

    fs.writeFileSync(filename, html);
};

processFile('index.html');
processFile('home.html');

console.log("Changed featured products to a horizontal scrolling carousel.");
